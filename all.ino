...
void handle_DFPlayer() {
  Serial.println("SFPLAYER");
    ...
    } else if (state == IDLE && buttonState == LOW && released) {
...
void handle_micro_switch() {
  Serial.println("handle micro switch");
  int leverState = digitalRead(leverPin);
  if (leverState == LOW && state > IDLE && state != WINNING_SONG_PLAYING) {
    state = WIN;
  }
}
...
void init_servo() {
  if (!TopServo.attach(pinTopServo) || !BottomServo.attach(pinBottomServo)) {
    ...
    while (true) {}                      // halt here so you notice
  }
